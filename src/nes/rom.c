...
// Use PSRAM for large allocations (ROM data)
static void* rom_malloc(size_t size) {
    void *ptr = heap_caps_malloc(size, MALLOC_CAP_SPIRAM);
    if (!ptr) ptr = malloc(size);  // Fallback to internal
    return ptr;
}
...
rom_t *rom_loadfile(const char *filename)
{
   ...
   if ((fp = fopen(filename, "rb")))
   {
      ...
      else if ((data = rom_malloc(size)) == NULL)
      {
         MESSAGE_ERROR("ROM: Memory allocation failed\n");
      }
