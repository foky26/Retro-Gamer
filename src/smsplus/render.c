...
// En render.c, añadir esta función después de render_line() o al final del archivo
void render_copy_palette(uint16_t *out)
{
    // pixel[] ya está en RGB565 con bytes swapped (línea 769 de render.c)
    // Deshacer el swap para devolver RGB565 normal
    for (int i = 0; i < PALETTE_SIZE; i++) {
        uint16_t c = pixel[i];
        out[i] = (c << 8) | (c >> 8);  // swap bytes de vuelta
    }
}